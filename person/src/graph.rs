...
// pub fn get_year_from_birthday(mut birthday: String) -> i32 {
//     match birthday.len() {
...
//             }
//         }
//         if children_year == 10000 { return 0 }
//         else { return children_year }
//     }else { return 0 }
// }
...
// fn search_year(person: Person) -> i32 {
//     let mut year: i32 = 0;
//     let all_parents: Vec<Person> = crate::person::get_all_parents(person.clone());
//     let all_children: Vec<Person> = crate::person::get_all_children(person.clone());
//     if all_children.len() > 0 {
//         all_children.into_iter()
//         .filter(|child| child.birthday.is_some())
//         .map(|child| child.birthday.unwrap())
//         .filter(|string| !string.is_empty())
//         .filter(|string| get_year_from_birthday(string.to_string()) < year)
//         .map(|string| year = get_year_from_birthday(string.to_string()));
//     }
//     println!("YEAR: {}", year);
//     year 
// }
...
//         None => { 
//             // function die das alles übernimmt also das suchen nach einem jahr und dann entsprechend anpasst
//             // zuerst die eigenen eltern und dann die letern des partners DANN die kinder 
//             // dann wird die funktion nochmal mit jedem aufgerufen der eine stufe drüber liegt also eigene eltern und partner eltern
//             // und das wird dann rekursiv, beim return dann immer +-25 
//             let ret_i32 = 0;//search_year(person.clone());
//             if ret_i32 != 0 { 
//                 return (translate(ret_i32-25), String::from("Unknown")) ; 
//             }else {
//                 return (SCALE_TO_UPPER, String::from("Unknown")) ;
//             }
//         },
//         Some(z) => {
//             if z.is_empty() { 
...
//                     if person.person_id == 41 { println!("RET : {}", ret_i32); }
//                     if ret_i32 != 0 { 
//                         return (translate(ret_i32+25), String::from("Unknown")) ; 
//                     }else{
//                         let parents: Vec<Person> = crate::person::get_all_parents(person.clone());
//                         for i in 0..parents.len() {
//                             let ret_i32 = get_year_of_parent(parents[i].clone());
//                             if ret_i32 != 0 {
//                                 return (translate(ret_i32+50), String::from("Unknown")) ; 
//                             }
//                         }
...
node [fontsize=11, fixedsize=true, height=1.5, width=1.5];\n\n");
    ...
    file.write_all(crate::matrix3::create_dot_string(id).as_bytes()).expect("write failed");
